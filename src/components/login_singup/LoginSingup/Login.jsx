...
export default function Login() {
  ...
  const getformdata = async (event) => {
    ...
    try {
      ...
      if(emailRef.current.value==="admin" && passRef.current.value==="12345678"){
        navigate("/admin")
      }else{
        navigate("/")
      }
      ...
      <div style={{ width: "100%", height: "auto", backgroundColor: "#f9f9fa",paddingBottom:"2rem" }}  >
        <form action="" onSubmit={getformdata} >
          ...
          <div className={styles.main}>
            ...
            <div>
               {" "}<RouterLink style={{textDecoration:"none"}} to={"/signup"}><span style={{color:"rgb(222, 87, 229)" , cursor: "pointer"}} >Create an Account</span></RouterLink>
